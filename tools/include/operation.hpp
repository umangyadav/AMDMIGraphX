...
auto compute_op(rank<1>, const T& x, const std::vector<argument>& input, const std::vector<module_ref>& module_args, F f)
    -> decltype(x.compute(input, module_args, f))
{
    return x.compute(input, module_args, f);
}

template <class T, class F>
argument compute_op(rank<0>, const T& x, const std::vector<argument>&, const std::vector<module_ref>&, F)
{
    std::string name = x.name();
    MIGRAPHX_THROW("Not computable: " + name);
}
...
template <class T, class F>
argument compute_op(const T& x, const std::vector<argument>& input, const std::vector<module_ref>& module_args, F f)
{
...
<%
 interface(
     ...
     virtual('compute',
             ...
             run = 'std::function<std::vector<argument>(module_ref& mdl, const std::vector<argument>& inputs)>',
             const   = True,
             default = 'detail::compute_op'),
