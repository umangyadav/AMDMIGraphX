...
struct onnx_parser
{
    ...
    onnx_parser()
    {
        ...
        add_mem_op("If", &onnx_parser::parse_if);
    ...
    instruction_ref make_contiguous(instruction_ref ins) const
    {
        ...
        return cur_modl->add_instruction(make_op("contiguous"), ins);
    }
    ...
    add_broadcastable_binary_op(instruction_ref arg0, instruction_ref arg1, const std::string& name)
    {
        if(arg0->get_shape().lens() != arg1->get_shape().lens())
        {
            ...
            if(arg0->get_shape().lens() != out_lens)
                l0 = cur_modl->add_instruction(op::multibroadcast{out_lens}, arg0);

            auto l1 = arg1;
            if(arg1->get_shape().lens() != out_lens)
                l1 = cur_modl->add_instruction(op::multibroadcast{out_lens}, arg1);

            return cur_modl->add_instruction(make_op(name), l0, l1);
        }
        else
        {
            return cur_modl->add_instruction(make_op(name), {arg0, arg1});
        }
    }
                        ...
                        bool contiguous = false)
    {
        add_op(
            ...
            [this, op_name, contiguous](const node_info& info, std::vector<instruction_ref> args) {
                ...
                return cur_modl->add_instruction(op, args);
            });
    }
                            ...
                            float pad_val         = 0) const
    {
        ...
        if(is_asym_padding(padding) or count_include_pad == 1)
        {
            ...
            ins = cur_modl->add_instruction(op::pad{asym_pads, pad_val}, ins);
        }
        else
        {
    ...
    parse_clip(const std::string&, node_info info, std::vector<instruction_ref> args) const
    {
        ...
        else if(contains(info.attributes, "min") and contains(info.attributes, "max"))
        {
            ...
            min_arg       = cur_modl->add_literal(min_val);
            max_arg       = cur_modl->add_literal(max_val);
            min_used      = true;
            max_used      = true;
        }
        ...
        if(min_used)
        {
            min_arg = cur_modl->add_instruction(op::multibroadcast{input_lens}, min_arg);
        }

        if(max_used)
        {
            max_arg = cur_modl->add_instruction(op::multibroadcast{input_lens}, max_arg);
        }

        if(min_used and max_used)
        {
            return cur_modl->add_instruction(make_op("clip"), args[0], min_arg, max_arg);
        }
        else if(max_used)
        {
            return cur_modl->add_instruction(make_op("min"), args[0], max_arg);
        }
        else if(min_used)
        {
            return cur_modl->add_instruction(make_op("max"), args[0], min_arg);
        }
        else
        {
            return cur_modl->add_instruction(make_op("identity"), args[0]);
        }
    }
                                 ...
                                 std::vector<instruction_ref> args) const
    {
        ...
        {
            return cur_modl->add_instruction(make_op(op_name, {{"axis", axis}}), std::move(args));
        }
    }
    ...
    instruction_ref reflect_pad(const std::vector<int64_t>& pads, instruction_ref input)
    {
        ...
        for(int64_t i = num_dims - 1; i >= 0; i--)
        {
            ...
            for(int idx : l_indices)
            {
                *starts_it = idx;
                *ends_it   = *starts_it + 1;
                slices.push_back(cur_modl->add_instruction(op::slice{axes, starts, ends}, input));
            }
            ...
            input = cur_modl->add_instruction(op::concat{axis}, slices);
        }
        return input;
    }
                               ...
                               std::vector<instruction_ref> args)
    {
        ...
        auto l1 = cur_modl->add_instruction(op, l0, args[1]);
    ...
    parse_conv_transpose(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        if(asym_padding)
        {
            ...
            l1 = cur_modl->add_instruction(op::slice{axes, starts, ends}, l1);
        }
        ...
        if(contains(info.attributes, "output_padding"))
        {
            ...
            l1 = cur_modl->add_instruction(op::pad{output_padding}, l1);
        }
        ...
        if(contains(info.attributes, "output_shape"))
        {
            ...
            if(curr_shape != output_shape)
            {
                ...
                l1 = cur_modl->add_instruction(op::pad{target_padding}, l1);
            }
        }
    ...
    parse_pooling(const std::string& name, node_info info, std::vector<instruction_ref> args)
    {
        ...
        op.from_value(values);
        auto l1 = cur_modl->add_instruction(op, l0);
        if(!slice_start.empty())
        {
            std::vector<int64_t> axes(kdims);
            std::iota(axes.begin(), axes.end(), 2);
            l1 = cur_modl->add_instruction(op::slice{axes, slice_start, slice_end}, l1);
        }
    ...
    parse_reshape(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        return cur_modl->add_instruction(op, make_contiguous(args[0]));
    }
    ...
    parse_resize(const std::string&, const node_info& info, std::vector<instruction_ref> args)
    {
        ...
        auto rsp     = cur_modl->add_instruction(make_op("reshape", {{"dims", rsp_lens}}), args[0]);
        auto ins_ind = cur_modl->add_literal(literal(ind_s, ind));
        return cur_modl->add_instruction(make_op("gather", {{"axis", 0}}), rsp, ins_ind);
    }
    ...
    parse_gather_elements(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        arg_data = cur_modl->add_instruction(op::reshape{{data_elem_num}}, arg_data);
    ...
    parse_slice(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        return cur_modl->add_instruction(op, args[0]);
    }
    ...
    parse_constant(const std::string&, node_info info, const std::vector<instruction_ref>&) const
    {
        ...
        {
            return cur_modl->add_literal(literal{});
        }
        ...
        if(dim_size == 0)
        {
            ...
            return cur_modl->add_literal(migraphx::literal{scalar_shape, v.data()});
        }

        return cur_modl->add_literal(v);
    }
    ...
    parse_gemm(const std::string&, node_info info, std::vector<instruction_ref> args) const
    {
        ...
        auto l1 = (transa) ? cur_modl->add_instruction(op::transpose{perm}, args[0]) : args[0];
        auto l2 = (transb) ? cur_modl->add_instruction(op::transpose{perm}, args[1]) : args[1];
        if(args.size() == 3)
        {
            if(beta != 0.f && args[2]->get_shape().elements() > 0)
            {
                ...
                {
                    l3 = cur_modl->add_instruction(op::multibroadcast{out_lens}, args[2]);
                }
                                 ...
                                 std::vector<instruction_ref> args)
    {
        ...
        if(l0_lens.size() == 1)
        {
            ...
            l0 = cur_modl->add_instruction(op::unsqueeze{{0}}, args[0]);
        }
        ...
        if(l1_lens.size() == 1)
        {
            is_b_appended = true;
            l1_lens.push_back(1);
            l1 = cur_modl->add_instruction(op::unsqueeze{{1}}, args[1]);
        }
        ...
        if(!std::equal(l0_lens.rbegin() + 2, l0_lens.rend(), l1_lens.rbegin() + 2, l1_lens.rend()))
        {
            ...
            {
                bl0 = cur_modl->add_instruction(op::multibroadcast{l0_broadcasted_lens}, l0);
            }
            if(l1_lens != l1_broadcasted_lens)
            {
                bl1 = cur_modl->add_instruction(op::multibroadcast{l1_broadcasted_lens}, l1);
            }
        }
        ...
        if(is_a_prepended)
        {
            dot_res = cur_modl->add_instruction(op::squeeze{{num_axis - 2}}, dot_res);
            --num_axis;
        }
        if(is_b_appended)
        {
            dot_res = cur_modl->add_instruction(op::squeeze{{num_axis - 1}}, dot_res);
        }
    ...
    parse_batchnorm(const std::string&, node_info info, std::vector<instruction_ref> args) const
    {
        ...
        return cur_modl->add_instruction(op, std::move(args));
    }
    ...
    parse_leaky_relu(const std::string&, node_info info, std::vector<instruction_ref> args) const
    {
        ...
        return cur_modl->add_instruction(op, args.front());
    }
    ...
    parse_transpose(const std::string&, node_info info, std::vector<instruction_ref> args) const
    {
        ...
        return cur_modl->add_instruction(migraphx::op::transpose{perm}, args.front());
    }
    ...
    instruction_ref parse_pad(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        {
            return cur_modl->add_instruction(make_op("identity"), args.front());
        }
        ...
        return cur_modl->add_instruction(migraphx::op::pad{pads, value}, args.front());
    }
    ...
    parse_selu(const std::string&, const node_info& info, std::vector<instruction_ref> args) const
    {
        ...
        auto l_alpha = cur_modl->add_literal({{type, {1}}, {alpha}});
        ...
        auto exp_x  = cur_modl->add_instruction(make_op("exp"), args[0]);

        auto alpha_ex  = cur_modl->add_instruction(make_op("mul"), l_alpha, exp_x);
        auto aex_alpha = cur_modl->add_instruction(make_op("sub"), alpha_ex, l_alpha);

        auto ins1 = cur_modl->add_instruction(make_op("add"), aex_alpha, args[0]);
        auto ins2 = cur_modl->add_instruction(make_op("sub"), aex_alpha, args[0]);

        auto sign2   = cur_modl->add_instruction(make_op("mul"), sign_x, ins2);
        auto ins_sub = cur_modl->add_instruction(make_op("sub"), ins1, sign2);

        return cur_modl->add_instruction(make_op("mul"), ins_sub, l_gamma);
    }
    ...
    parse_shape(const std::string&, const node_info&, std::vector<instruction_ref> args) const
    {
        ...
        return cur_modl->add_literal(migraphx::literal{s, vec_shape});
    }
    ...
    parse_constant_fill(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        if(input_as_shape == 1)
        {
            ...
            return cur_modl->add_literal(migraphx::literal(s, values));
        }
    ...
    parse_constant_of_shape(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        else
        {
            ...
            return cur_modl->add_literal(l_out);
        }
    }
    ...
    parse_expand(const std::string&, const node_info&, std::vector<instruction_ref> args)
    {
        ...
        return cur_modl->add_instruction(op::multibroadcast{out_lens}, args[0]);
    }
    ...
    parse_rnn(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        if(args.size() < 6)
        {
            auto ins = cur_modl->add_instruction(op::undefined{});
                                      ...
                                      std::vector<instruction_ref> args) const
    {
        ...
        if(keep_dims == 1)
        {
            return cur_modl->add_instruction(make_op(op_name, {{"axes", axes}}), std::move(args));
        }
        else
        {
    ...
    {
        auto abs_ins = cur_modl->add_instruction(make_op("abs"), args[0]);
    ...
    {
        auto square_ins = cur_modl->add_instruction(make_op("mul"), args[0], args[0]);
        auto sum_ins    = parse_reduce_oper({}, "reduce_sum", std::move(info), {square_ins});
        return cur_modl->add_instruction(make_op("sqrt"), sum_ins);
    }
                                         ...
                                         std::vector<instruction_ref> args) const
    {
        ...
        return cur_modl->add_instruction(make_op("log"), sum_ins);
    }
    ...
    instruction_ref parse_reduce_log_sum_exp(const std::string&,
                                             ...
                                             std::vector<instruction_ref> args) const
    {
        auto exp_ins = cur_modl->add_instruction(make_op("exp"), args[0]);
    ...
    parse_split(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        {
            ret_ins.push_back(
                cur_modl->add_instruction(op::slice{{axis}, {start}, {start + sl}}, args[0]));
            start += sl;
        }
    ...
    parse_onehot(const std::string&, node_info info, std::vector<instruction_ref> args)
    {
        ...
        auto gather_out = cur_modl->add_instruction(op::gather{0}, {l_val, args[0]});
        ...
        auto tr_out = cur_modl->add_instruction(op::transpose{perm}, gather_out);
        auto lens   = tr_out->get_shape().lens();

        auto off_val       = cur_modl->add_instruction(op::slice{{0}, {0}, {1}}, args[2]);
        auto on_val        = cur_modl->add_instruction(op::slice{{0}, {1}, {2}}, args[2]);
        auto diff          = cur_modl->add_instruction(make_op("sub"), on_val, off_val);
        auto unsq_off_val  = cur_modl->add_instruction(op::multibroadcast{lens}, off_val);
        auto unsq_diff_val = cur_modl->add_instruction(op::multibroadcast{lens}, diff);
    ...
    parse_tile(const std::string&, const node_info&, std::vector<instruction_ref> args)
    {
        ...
        for(int i = 0; i < repeats.size(); i++)
        {
            ...
            {
                l0 = cur_modl->add_instruction(op::concat{i}, l0, l1);
            }
        }
        return l0;
    }
                                        ...
                                        std::vector<instruction_ref> args) const
    {
        ...
        auto l0 = cur_modl->add_instruction(op::gather{}, args[0], args[1]);
        switch(reduce_mode)
        {
        case reduce_mode_t::sum:
            l0 = cur_modl->add_instruction(make_op("reduce_sum", {{"axes", {0}}}), l0);
            break;
        case reduce_mode_t::mean:
            l0 = cur_modl->add_instruction(make_op("reduce_mean", {{"axes", {0}}}), l0);
            break;
        case reduce_mode_t::max:
            l0 = cur_modl->add_instruction(make_op("reduce_max", {{"axes", {0}}}), l0);
            break;
        }
        return l0;
    }
    ...
    {
        auto out = cur_modl->add_instruction(make_op("identity"), args[0]);
        auto s   = args[0]->get_shape();
        std::vector<int8_t> vec(s.elements(), 1);
        shape mask_s{shape::bool_type, s.lens()};
        auto mask = cur_modl->add_literal(literal(mask_s, vec));
    ...
    parse_nonzero(const std::string&, const node_info&, std::vector<instruction_ref> args)
    {
        ...
        return cur_modl->add_literal(literal(out_s, out_data));
    }
    ...
    parse_where(const std::string&, const node_info&, std::vector<instruction_ref> args)
    {
        ...
        auto l_ind = cur_modl->add_literal(literal(ind_s, ind));
        std::vector<int> offset(elem_num, elem_num);
        auto l_offset   = cur_modl->add_literal(literal({shape::int32_type, lens}, offset));
        auto ins_offset = cur_modl->add_instruction(make_op("mul"), l_offset, cond);
        auto ins_ind    = cur_modl->add_instruction(make_op("add"), ins_offset, l_ind);

        return cur_modl->add_instruction(make_op("gather", {{"axis", 0}}), rsp_data, ins_ind);
    }

    std::vector<instruction_ref>
    parse_if(const std::string&, const node_info& info, std::vector<instruction_ref> args)
    {
        auto arg_cond = args[0]->eval();
    ...
    void parse_graph(const onnx::GraphProto& graph)
    {
        ...
        {
            instructions[f.name()] = cur_modl->add_literal(parse_tensor(f));
        }
        ...
        for(auto&& input : graph.input())
        {
            ...
            if(!contains(instructions, name))
            {
                ...
                instructions[name] = cur_modl->add_parameter(name, s);
            }
        }
        ...
        for(auto&& node : graph.node())
        {
            ...
            if(ops.count(node.op_type()) == 0)
            {
                if(skip_unknown_operators)
                    result.push_back(cur_modl->add_instruction(op::unknown{node.op_type()}, args));
                else
        ...
        cur_modl->add_return(output_ins);
    }
