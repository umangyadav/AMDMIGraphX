...
argument hip_iff::compute(context& ctx, const shape&, std::vector<argument> args, 
            std::vector<module_ref>& modules,
            std::function<std::vector<argument>(const module_ref& mdl, context& ctx, 
            const std::vector<argument>& inputs)> run) const
{
