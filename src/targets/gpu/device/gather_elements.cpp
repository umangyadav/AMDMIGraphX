...
void gather_elements(hipStream_t stream,
                     const argument& result,
                     const argument& arg_data,
                     const argument& arg_ind,
                     int axis)
{
    ...
    hip_visit_all(result, output_shape)([&](auto out, auto out_s) {
        auto* out_data = device_cast(out.data());
        hip_visit_all(arg_data, input_shape)([&](auto in, auto in_s) {
